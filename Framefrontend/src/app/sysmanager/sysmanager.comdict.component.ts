...
@Component({
  selector: 'app-sysmanager-comdict',
  templateUrl: './sysmanager.comdict.component.html',
  styleUrls: ['./sysmanager.comdict.component.css']

})
export class SysmanagerComdictComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getComdictsName();

  }

  //获取全部公共字典
  getComdictsName(): void {

    let reqdata: RequestParams = new RequestParams();
    let currreqobj = { params: "" };
    reqdata.AddrType = "FrameMgt";
    reqdata.GetDataUrl = "/FrameApi/GetFrameDictAllN";
    reqdata.Reqmethod = "post";
    reqdata.Reqdata = "";
    reqdata.ReqdataType = "";
    reqdata.Reqtype = "getframedicts";
    reqdata.Reqi18n = this.commonmodule.getI18nlang();
    reqdata.ServIdentifer = "frameManager";
    reqdata.ServVersionIden = "1.0";

    this.sysmanagerServ.getFrameDicts(reqdata,
      res => this.getFrameDictsResdataFunc(res), res => this.getFrameDictsDisplaydataFunc(res), err => this.getFrameDictsErrorFunc(err));
  }
  getFrameDictsResdataFunc(data: any): any {
    return data;
  }
  getFrameDictsDisplaydataFunc(data: any): void {
    if (data.status == 200) {
  ...
  getFrameDictsErrorFunc(err: string) {
    this.errorinfos = [];
  ...
  getData(currpageindex: number): void {
    ...
    { queryItemKey: "SSY_FRAME_DICT", queryItemValue: JSON.stringify(currqueryobj) }];

    let reqdata: RequestParams = new RequestParams();
    reqdata.AddrType = "FrameMgt";
    reqdata.GetDataUrl = "/FrameApi/GetFrameDictPagerN";
    reqdata.Reqmethod = "post";
    reqdata.Reqdata = JSON.stringify(groupreqobj);
    reqdata.ReqdataType = "";
    reqdata.Reqtype = "getsystemdictdata";
    reqdata.Reqi18n = this.commonmodule.getI18nlang();
    reqdata.ServIdentifer = "frameManager";
    reqdata.ServVersionIden = "1.0";

    this.sysmanagerServ.getSystemDictData(reqdata,
      res => this.getSystemDictDataResdataFunc(res), res => this.getSystemDictDataDisplaydataFunc(res), err => this.getSystemDictDataErrorFunc(err));
  }

  getSystemDictDataResdataFunc(data: any): any {
    return data;
  }
  getSystemDictDataDisplaydataFunc(data: any): void {
    this.condiv = false;
    this.bindcurrdata = [];
    this.tottlecounts = 0;
    this.currdata = [];
  ...
  getSystemDictDataErrorFunc(err: string) {
    this.errorinfos = [];
    this.errorinfos.push({ severity: 'warn', summary: this.commonlang.noticeInfoTitle, detail: this.commonlang.noticeHttpError_info + err });
    this.condiv = false;
  }
  ...
  addData(): void {
    ...
    if (this.selectrows.length > 0 && this.isTmp) {
      for (let i = 0; i < this.selectrows.length; i++) {
        ...
        newd_entity.SSY_FRAME_DICTID = "" + i + Math.floor(Math.random() * (32 + 1));
      ...
      let newd_entity: any = { SSY_FRAME_DICTID: "", DOMAINNAMEIDEN: "", DOMAINNAMES: "", OPTIONIDEN: "", OPTIONNAMES: "" };
      newd_entity.SSY_FRAME_DICTID = "tmp" + Math.floor(Math.random() * (10000 + 1));
  ...
  saveData(): void {
    ...
    for (let i = 0; i < this.currdata.length; i++) {
      if (this.currdata[i]["SSY_FRAME_DICTID"].substring(0, 3) == "tmp") {
        this.currdata[i]["SSY_FRAME_DICTID"] = "";
      }
    }
    //先比较变更结果，只提交有变更的数据,该函数处理比较结果并copy到submitdata数组
    let mainkeys: MainValueName[] = [{ PropName: "SSY_FRAME_DICTID" }];
    ...
    if (checkNewData.length > 0) {
      ...
      let bizobjop = [{ queryItemKey: "bizObjectName", queryItemValue: "SSY_FRAME_DICT" },
      ...
      { queryItemKey: "opObjPropertyL", queryItemValue: JSON.stringify(["SSY_FRAME_DICTID", "DOMAINNAMEIDEN", "DOMAINNAMES", "OPTIONIDEN", "OPTIONNAMES", "OPTIONIDEN_CUT", "OPTIONNAMES_CUT", "PYM", "REMARKS", "TIMESTAMPSS"]) },
      { queryItemKey: "wherePropertyL", queryItemValue: JSON.stringify(["SSY_FRAME_DICTID"]) },
      { queryItemKey: "mainPropertyL", queryItemValue: JSON.stringify(["SSY_FRAME_DICTID"]) }
      ];
      let reqdata: RequestParams = new RequestParams();
      reqdata.AddrType = "FrameMgt";
      reqdata.GetDataUrl = "/FrameApi/OpBizObjectSingle_SSY_FRAME_DICTN";
      reqdata.Reqmethod = "post";
      reqdata.Reqdata = JSON.stringify(bizobjop);
      reqdata.ReqdataType = "";
      reqdata.Reqtype = "savesystemdictdata";
      reqdata.Reqi18n = this.commonmodule.getI18nlang();
      reqdata.ServIdentifer = "frameManager";
      reqdata.ServVersionIden = "1.0";

      this.sysmanagerServ.saveSystemDictData(reqdata,
        res => this.saveSystemDictDataResdataFunc(res), res => this.saveSystemDictDataDisplaydataFunc(res), err => this.saveSystemDictDataErrorFunc(err));
  ...
  saveSystemDictDataResdataFunc(data: any): any {
    return data;
  }
  saveSystemDictDataDisplaydataFunc(data: any): void {
    this.consavediv = false;
    try {
      if (data.status == 200) {
        let respdata: ResponseParams = JSON.parse(JSON.parse(data._body));
  ...
  saveSystemDictDataErrorFunc(err: string) {
    this.consavediv = false;
    this.errorinfos = [];
    this.errorinfos.push({ severity: 'warn', summary: this.commonlang.noticeInfoTitle, detail: this.commonlang.noticeHttpError_info + err });
  }
